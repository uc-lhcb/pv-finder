...
def getLinRegDatasets(num_train, num_test, x_dim=1, y_dim=1, m_var=0.25, b_var=0.25, preset=(False, 0, 0)):
    ...
    x_train = np.concatenate((np.square(x_train),x_train),axis=-1)
    x_test = np.concatenate((np.square(x_test),x_test),axis=-1)
...
#returns data in form approx y = ax^2 + mx + b, where a = 10 for now
