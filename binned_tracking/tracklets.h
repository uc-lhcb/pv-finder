...
const Hit& getHit(const HitIdx &h){
  return HitArray::instance()->at(h.sensor,h.phi_bin,h.idx);
}
...
public:
  ...
  Tracklet(int n, HitIdx hits[]){
    _n = n;
    for(int i=0; i<n; i++) _hits[i]=hits[i];
    _init();
  }
  ...
  static void regression(int nhits, HitIdx hits[], double &x0, double &y0, double &z0,
...
void makeTracklets(int marked[SENSORS][PHI_BINS][MAX_HITS_PER_BIN],
  int &ntracks, Tracklet tracks[]){
  ...
  for (int sens0 = SENSORS-1; sens0 >= 2; --sens0) {
    ...
    for(int phi0=0; phi0 < PHI_BINS; phi0++){
      ...
      for(int i0=0; i0<hit_array->n(sens0,phi0); i0++){
        if(marked[sens0][phi0][i0]) continue;
        ...
        for(int k=0; k<2; k++){
          ...
          for(int i1=min; i1<hit_array->n(sens1,phi1); i1++){
            ...
            if(hit1.r() > rMax){
              if(phi1==phi0) i1min=i1+1;
              continue;
            }
            if(hit1.r() < rMin) break;
            if(marked[sens1][phi1][i1]) continue;
            if(deltaPhi(hit0.phi(),hit1.phi()) > 0.08) continue;
            Trajectory seed(hit0.point(),hit1.point());
            if(!seed.goodVeloSlopes()) continue;  // cuts on tx and ty
            ...
            while (next >= 0) {
              ...
              if(i2 >= 0){
                ...
                seed = Trajectory(getHit(hit_buffer[foundHits-2]).point(),getHit(hit_buffer[foundHits-1]).point());
              }
              else { // no hits found
                if(step == 2){  // look on the other side
                  hit_array->bestHit(next+1,getHit(hit_buffer[foundHits-1]),seed,phi2,i2,marked);
                  if(i2 >= 0){
              ...
              if(1 < nbMissed) break; // default is 2, change to 1
              next -= step;
            }
            if(foundHits < 3) continue; // no tracklet
            if(foundHits == 3){ // triplet (10% gain by not treating these special)
              triplets[ntriplets] = Tracklet(3,hit_buffer);
              if(triplets[ntriplets].chi2NDof() > 20/3.) continue;
              ntriplets++;
              continue;
            }
            unsigned unUsed = 0;
            for(int h=0; h<foundHits; h++){
              ...
              if(marked[hidx.sensor][hidx.phi_bin][hidx.idx] == 0) unUsed++;
            }
            if(2*unUsed < foundHits * FRAC_UNUSED) continue;
