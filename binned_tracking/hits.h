...
int getPhiBinNN(double phi){
  ...
  int nn = phi_bin + 1;
  if(dmin < dmax) nn = phi_bin - 1;
  if(nn < 0) nn = PHI_BINS-1;
  if(nn > PHI_BINS-1) nn=0;
  return nn;
}
...
private:
  ...
  int _zo[26] = {-277, -252, -227, -202, -132, -62, -37, -12, 13, 38, 63, 88, 113, 138, 163, 188, 213, 238, 263, 325, 402, 497, 616, 661, 706, 751};
  int _ze[26] = {-289, -264, -239, -214, -144, -74, -49, -24, 1, 26, 51, 76, 101, 126, 151, 176, 201, 226, 251, 313, 390, 485, 604, 649, 694, 739};

  // make it a singleton
  static HitArray *_instance;
  HitArray(){
    for(int s =0; s<SENSORS; s++){
      for(int i=0; i<PHI_BINS; i++) _hits[s][i] = new Hit[MAX_HITS_PER_BIN];
    }
  }
  ...
  void _reset() {
    for(int s=0; s<SENSORS; s++) {
      for(int i=0; i<PHI_BINS; i++) _n[s][i]=0;
    }
  }
  ...
  void _addHit(Data &data, int h) {
    ...
    if(_n[s][i] == MAX_HITS_PER_BIN-1) return; // just ignore it (obviously not a great solution, but OK for this test)
...
public:
  ...
  static HitArray* instance() {
    if(!_instance) _instance = new HitArray();
    return _instance;
  }
  ...
  void newEvent(Data &data){
    ...
    _reset();
    for(int h=0; h<nhits; h++) _addHit(data,h);
    // sort hits
    for(int s=0; s<SENSORS; s++) {
      for(int i=0; i<PHI_BINS; i++){
  ...
  double zsensor(int s) const {
    if(s % 2 == 0) return _ze[s/2];
    else return _zo[s/2];
  }

  void bestHit(int s, const Hit &hit1, const Trajectory &traj, int &ii, int &jj, int marked[SENSORS][PHI_BINS][MAX_HITS_PER_BIN]) const {
    ii=-1; jj=-1;
    if(_n[s][hit1.phiBin()] < 1 && _n[s][hit1.phiBinNN()] < 1) return;
    ...
    for(int k=0; k<2; k++){
      ...
      if(_n[s][bin] < 1) continue;
      if(_hits[s][bin][0].r() < rMin) continue;
      if(_hits[s][bin][_n[s][bin]-1].r() > rMax) continue;
      for(int j=0; j<_n[s][bin]; j++){
        const Hit &hit = _hits[s][bin][j];
        if(hit.r() > rMax) continue;
        if(hit.r() < rMin) break;
        if(marked[s][bin][j]) continue;
        if(deltaPhi(hit1.phi(),hit.phi())>0.08) continue;
