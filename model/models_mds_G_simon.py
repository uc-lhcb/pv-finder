...
## same as SimpleCNN5Layer_Ca, except that 2 channels of input
## are used (presumably poca_KDE_A and poca_KDE_B).
## with Softplus activation
    softplus = torch.nn.Softplus()
    def __init__(self):
        super(SimpleCNN5Layer_Ca_Two_KDE, self).__init__()
    ...
    def forward(self, x):
        ...
        return x
class SimpleCNN7Layer_Ca_Two_KDE(nn.Module):
## same as SimpleCNN5Layer_Ca_Two_KDE, except that 
## the number of channel per layer is increased to match 
## FourFeature_CNN6Layer_A and the number of layers is
## increased by 2.
    softplus = torch.nn.Softplus()
    def __init__(self):
        super(SimpleCNN7Layer_Ca_Two_KDE, self).__init__()
    ...
    def forward(self, x):
        ...
        x = leaky(self.conv5(x))
        x = self.conv5dropout(x)
        x = leaky(self.conv6(x))
        x = self.conv6dropout(x)
        x = leaky(self.conv7(x))
...
## significant structure in the 4K x 4K matrix
    ...
    def forward(self, x):
        ...
        return x
########## -- end of AllCNN8Layer_Ca_TwoKDE ----
#############  --  begin Two_KDE all CNN  with perturbative features ########
class AllCNN8Layer_Ca_Two_KDE_withPfc(nn.Module):
## same as SimpleCNN7Layer_Ca_Two_KDE, except that 
## the fully connected layer at the end is replaced with a CNN layer
    softplus = torch.nn.Softplus()
    def __init__(self):
        super(AllCNN8Layer_Ca_Two_KDE_withPfc, self).__init__()
...
class TwoFeature_CNN6Layer_A(nn.Module):
    ...
    def __init__(self):
        ...
        assert self.conv4of6.kernel_size[0] % 2 == 1, "Kernel size should be odd for 'same' conv."
        ...
        assert self.conv5of6.kernel_size[0] % 2 == 1, "Kernel size should be odd for 'same' conv."
...
## should have a different name than use in TwoFeature_CNN6Layer_A
        ...
        assert self.conv1_4Features.kernel_size[0] % 2 == 1, "Kernel size should be odd for 'same' conv."
...
## with Softplus activation
    softplus = torch.nn.Softplus()
    def __init__(self):
        super(AllCNN6Layer_Ca, self).__init__()
...
## mds         self.fc1 = nn.Linear(
## mds             in_features = 4000 * self.conv5.out_channels,
