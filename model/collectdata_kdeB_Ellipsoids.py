    ...
    **kargs: Any other keyword arguments will be passed on to torch's DataLoader
    ...
    for XY_file in files:
        ...
        with Timer(msg), h5py.File(XY_file, mode="r") as f:
            ...
            Xmax = 2500.0 * np.asarray(f["poca_KDE_B_xMax"])
            Ymax = 2500.0 * np.asarray(f["poca_KDE_B_yMax"])
...
def load_prebuilt_dataset(batch_size=16, num_files=1):
    ...
    for i in range(1, num_files + 1):
        tracks.append(
            torch.Tensor(
                np.load(
                    f"/share/lazy/will/data/tracks_to_KDE/June30_2020_80k_{i}_tracks.h5.npy"
                )
            )
        )
        KDEs.append(
            torch.Tensor(
                np.load(
                    f"/share/lazy/will/data/tracks_to_KDE/June30_2020_80k_{i}_KDEs.h5.npy"
                )
            )
        )
        PVs.append(
            torch.Tensor(
                np.load(
                    f"/share/lazy/will/data/tracks_to_KDE/June30_2020_80k_{i}_PVs.h5.npy"
                )
            )
        )
    ...
    return torch.utils.data.DataLoader(
        torch.utils.data.TensorDataset(torch.cat(tracks), torch.cat(KDEs)),
