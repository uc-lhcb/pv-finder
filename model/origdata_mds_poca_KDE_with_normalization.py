...
def process_root_file(filepath, sd_1=0.1):
    ...
    with Timer(start=f"Loading file: {name}"):
        ...
        slope = np.abs(
            np.divide(perp, major_axis_z)
        )  ## take abs as this will be a scaling factor
        ...
        m1_mag = np.sqrt(
            np.square(minor_axis1_x)
            + np.square(minor_axis1_y)
            + np.square(minor_axis1_z)
        )
        ...
        m2_mag = np.sqrt(
            np.square(minor_axis2_x)
            + np.square(minor_axis2_y)
            + np.square(minor_axis2_z)
        )
        for iEvt in range(20):
            print("iEvt = ", iEvt)
            print(
                "recon_x & poca_x[iEvt][0]      = ", recon_x[iEvt][0], poca_x[iEvt][0]
            )
            print(
                "recon_y & poca_y[iEvt][0]      = ", recon_y[iEvt][0], poca_y[iEvt][0]
            )
            print(
                "recon_z & poca_z[iEvt][0]      = ", recon_z[iEvt][0], poca_z[iEvt][0]
            )
        ...
        for iEvt in range(10):
            ...
            plt.hist(
                logSigmaZ, bins=100, range=[-5.0, 5.0], density=False, facecolor="b"
            )
            ...
            plt.plot(
                ...
                label="poca_z v pocay",
                linestyle="None",
            )
            plt.legend(loc="upper right")
            plt.axis([-100.0, 200.0, -0.10, 0.10])
            ...
            plt.plot(
                ...
                label="poca_z v poca_r",
                linestyle="None",
            )
            plt.legend(loc="upper right")
            plt.axis([-100.0, 200.0, 0.00, 0.14])
            plt.xlabel("poca_z", fontsize=14)
            ...
            good_major_axis_perp = np.sqrt(
                np.square(good_major_axis_x) + np.square(good_major_axis_y)
            )
            plt.plot(
                good_major_axis_perp,
                good_major_axis_z,
                color="b",
                marker="o",
                markersize=4,
