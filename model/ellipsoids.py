...
def six_ellipsoid_parameters(majorAxis, minorAxis_1, minorAxis_2):
    ...
    D = (
        np.multiply(u1[:, 0], u1[:, 1])
        + np.multiply(u2[:, 0], u2[:, 1])
        + np.multiply(u3[:, 0], u3[:, 1])
    )
    E = (
        np.multiply(u1[:, 2], u1[:, 0])
        + np.multiply(u2[:, 2], u2[:, 0])
        + np.multiply(u3[:, 2], u3[:, 0])
    )
    F = (
        np.multiply(u1[:, 1], u1[:, 2])
        + np.multiply(u2[:, 1], u2[:, 2])
        + np.multiply(u3[:, 1], u3[:, 2])
    )
