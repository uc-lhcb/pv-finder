...
def plot_ruiplot(zvals, i, inputs, labels, outputs, width=25, ax=None, styles=RUI_STYLES):
    ...
    with plt.rc_context(mystyle):
        ...
        ax.bar(x_bins, y_kernel,
               width=0.1, **styles['kernel'], label='Kernel Density')
        ...
        p1=ax_prob.bar(x_bins, y_target,
                       width=0.1, **styles['target'], label='Target')
        p2=ax_prob.bar(x_bins, y_predicted,
                       width=0.1, **styles['predicted'], label='Predicted')
        ...
        if np.any(np.isnan(labels)):
            ...
            ax_prob.bar(x_bins, grey_y,
               width=0.1, **styles['masked'], label='Masked')
