...
def foil(run, half, plane, u, vmin, vmax):
    g = ROOT.TGraph(1000); g.SetLineWidth(2)
    dv = (vmax - vmin)/1000.0
    for i in range(0, 1000):
        v = vmin + dv + i*dv
        if plane == 'xy': g.SetPoint(i, v, fms[run].x(v, u, half))
        else: g.SetPoint(i, v, fms[run].x(u, v, half))
...
def module(run, half, plane, u, vmin, vmax):
    ...
    if plane == 'xy':
        ...
        for i in range(0, 1000):
            ...
            if up[1] > mms[run].x(s, uv, -1): ups += [up]
            if lp[1] < mms[run].x(s, lv, 1): lps += [lp]
        ps = ups + lps + [ups[0]]
        g = ROOT.TGraph(len(ps))
        for i, (x, y) in enumerate(ps): g.SetPoint(i, x, y)
    else:
        n, g = 0, ROOT.TGraph((mms[run].sensor(800) + 1)*3/2)
        for s in range(0, mms[run].sensor(800) + 1):
            if mms[run].x(s, 0)*half < 0: continue
            z = mms[run].z(s)
...
def plotY(run, y, zmin = -300, zmax = 800):
    ...
    frm.GetXaxis().SetTitle('#it{z} [mm]'); frm.GetXaxis().CenterTitle()
    frm.GetYaxis().SetTitle('#it{x} [mm]'); frm.GetYaxis().CenterTitle()
    frm.GetYaxis().SetTitleOffset(0.5); frm.SetTitle('#it{y} = %i mm' % y)
    for g in gs: g.Draw('SAME L')
    cnv.SetCanvasSize(1200, 600)
    cnv.SetLeftMargin(0.05); cnv.SetRightMargin(0.05);
...
def plotZ(run, z, ymin = -40, ymax = 40, mod = True):
    ...
    if mod: gs += [module(run, 0, 'xy', z, ymin, ymax)]
    gs[0].Draw()
    cnv = ROOT.gPad.GetCanvas()
    frm = cnv.DrawFrame(ymin, ymin, ymax, ymax)
    frm.GetXaxis().SetTitle('#it{y} [mm]'); frm.GetXaxis().CenterTitle()
    frm.GetYaxis().SetTitle('#it{x} [mm]'); frm.GetYaxis().CenterTitle()
