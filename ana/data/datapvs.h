...
template <class T> struct DataPVs2IO : public CoreIO<T, float> {
    ...
    Vec nverts_before{this, "nverts_before"};
    Vec nverts_after{this, "nverts_after"};
...
inline void copy_in_pvs2(DataPVsOut2 &self,
    ...
    // initialize new prt_pvr and fill with -1 (initially no assignment)
    std::vector<int> prt_pvr_new(data_trks.prt_pvr->size(),-1);
...
//             reassigned_pointers.push_back(current_ind);
        ...
        if (!pvr_z_current.empty()) {
            ...
            self.ntrks->push_back(std::accumulate(current_numtracks.begin(),
                                                 current_numtracks.end(),
                                                 decltype(current_numtracks)::value_type(0)));
            
            self.pvr_x->push_back(weightedavg_x);
            self.pvr_y->push_back(weightedavg_y);
            self.pvr_z->push_back(weightedavg_z);
            
            self.nverts_before->push_back(nv_before);
            self.nverts_after->push_back(nv_after);
            
            self.ntrks_prompt->push_back(std::accumulate(current_ntrks_prompt.begin(),
    ...
    for(int i = 0; i < prt_pvr_new.size(); ++i) {
        self.prt_pvr->push_back(prt_pvr_new.at(i));
    }
