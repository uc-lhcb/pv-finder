...
//by  b_branchname->GetEntry(ientry); //read only this branch
  ...
  cout << "getting started;  sanity check" << endl;
...
// mds    cout <<  "in shiftx_list loop with shift_index = " << shift_index 
...
// mds        <<  shiftStr << endl;
  ...
  cout << "exited shiftx loop" << endl << endl;
...
/*
     if (jentry < 5) {
      cout << "  jentry =  " << jentry << endl << endl;
      for (size_t i=0; i<PrV_ParticleIndices.size(); i++) {
        cout << " vertex " << i << " has " <<  PrV_ParticleIndices[i].size() 
             << " particles:  "  << endl;
        for (size_t j=0; j<PrV_ParticleIndices[i].size(); j++) {
          cout << PrV_ParticleIndices[i][j] << " ";
        }
         cout << endl;
       }
     }
...
/*
     if (jentry < 5) {
      ...
      for (size_t i=0; i<Particle_HitIndices.size(); i++) {
        cout << " particle " << i << " has " <<  Particle_HitIndices[i].size()
             << " hits:  "  << endl;
        for (size_t j=0; j<Particle_HitIndices[i].size(); j++) {
          cout << Particle_HitIndices[i][j] << " ";
        }
         cout << endl;
       }
     }
...
/*  for testing & debugging
        if (jentry < 5) {
          cout << " event " << jentry << "  prv_index = " << prv_index
               << "  PrV_ParticleIndices[prv_index].size() = "
               <<  PrV_ParticleIndices[prv_index].size() << endl;
        }
*/
...
/*  for testing & debugging
            if (jentry<5) {
              cout << "j = " << j  << "  prt_index = " << prt_index
                   << "  Particle_HitIndices[prt_index].size() = "
                   << Particle_HitIndices[prt_index].size() << endl;
            }
*/
...
/*
                if (jentry<5) {
                cout << "k = " << k << "  hit_index = " << hit_index
                     << "  hit_phi = " << hit_phi << endl;
                }
*/
...
/*
                if (jentry < 5) {
                  cout << " nHits = " << nHits << "  hit_phi = " << hit_phi
                       << "  delta_phi = " << delta_phi
                       << "  delta_phi_sq = " << delta_phi*delta_phi 
                       << "  sum_delta_phi_sq = " << sum_delta_phi_sq << endl;
                 }
*/
...
/*
          if (jentry < 5) {
            cout << "ave_delta_phi_sq = " << ave_delta_phi_sq << "  delta_phi_rms = "
                 << delta_phi_rms << endl << endl;
          }
*/
