...
class Ellipsoid {
  ...
  public:
    ...
    Ellipsoid(const Trajectory& t1, const Trajectory& t2, const double& xy_error, const double& z_error, const double& road_error = 0) {
      ...
      auto const yhat    = 1/doca*v3;
      auto const v2_mag  = std::sqrt(v2.Dot(v2));
      auto const zhat    = 1/v2_mag*v2;
      auto const xhat    = yhat.Cross(zhat);
      if(road_error==0){
          _minor_axis1       = xy_error*yhat;
          _minor_axis2       = xy_error*xhat;
          _major_axis        = z_error*zhat;
