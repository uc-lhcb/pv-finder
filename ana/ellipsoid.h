...
class Ellipsoid {
  ...
  public:
    ...
    Ellipsoid(const Trajectory& t1, const Trajectory& t2, const double& xy_error, const double& z_error, const double& road_error = 0) { // EMK (change back to 0!!!)
      // road error 0.0566
      ...
      if(road_error==0){
          _minor_axis1       = scale_factor*xy_error*yhat; // EMK (before road_error*zhat)
          _minor_axis2       = scale_factor*xy_error*xhat; // EMK (before road_error*yhat)
          _major_axis        = scale_factor*z_error*zhat;
      }
      else{
          _minor_axis1       = road_error*yhat;
          _minor_axis2       = road_error*xhat;
          _major_axis        = (road_error/std::tan(v1.Angle(v2)))*zhat;
      }
