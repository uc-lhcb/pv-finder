...
public:
  ...
  Trajectory(double x, double y, double z, double tx, double ty):
  _point(x,y,z),_tx(tx),_ty(ty){}
  ...
  Trajectory(const Point &p1, const Point &p2){
    ...
    double dz = p2.z()-p1.z();
    if(dz < 1e-3){ // requires hits in same module
      _tx = 666; _ty = 666;
    }
    else{
      _tx = (p2.x()-p1.x())/dz;
  ...
  bool goodVeloSlopes() const {
    if(abs(_tx) > 0.4 || abs(_ty) > 0.4) return false;
    else return true;
  }
